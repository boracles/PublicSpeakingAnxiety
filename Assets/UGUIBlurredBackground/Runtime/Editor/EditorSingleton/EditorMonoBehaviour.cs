...
namespace Kamgam.UGUIBlurredBackground.EditorSingleton
{
    ...
    public class EditorMonoBehaviour : MonoBehaviour
    {
        ...
        public void Update()
        {
			...
			// Fix for symptoms of drag issues in color picker (needs further investigation).
#if UNITY_EDITOR
            if (!EditorApplication.isPlayingOrWillChangePlaymode && IsColorPickerFocused())
                return;
#endif
...
#if UNITY_EDITOR
        static bool IsColorPickerFocused()
        {
            var focusedWindow = EditorWindow.focusedWindow;
            return focusedWindow != null && focusedWindow.GetType().FullName == "UnityEditor.ColorPicker";
        }
#endif
